...
name=Chef YUM spec testing repo
  ...
  describe ":install" do
    ...
    context "downgrades" do
      ...
      it "throws a deprecation warning with allow_downgrade" do
        ...
        expect(Chef).to receive(:deprecated).with(:yum_package_allow_downgrade, /^the allow_downgrade property on the yum_package provider is not used/)
        preinstall("chef_rpm-1.10-1.fc24.x86_64.rpm")
        yum_package.version("1.2")
        yum_package.run_action(:install)
